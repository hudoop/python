...
for i in range(1,20):#从网站上直接获取页面个数
    ...
    for contenttd in alllist:
        row=[]
        mouth=contenttd.find('td',width="115").text#定位到宣讲时间字符位置
        companyweb=contenttd.find('td',width='140').find('a').get('href')#定位到宣讲企业网站
        ...
        companyname=contenttd.find('td',width='140').find('a').text#定位宣讲企业名字
        xuexiao=contenttd.find('td',width='140').next_sibling.next_sibling.text#定位宣讲学校名字，这里第一个兄弟节点通常是空，所以再定位一次兄弟节点。具体可以看BeautifulSoup官方文档有解释
        jiaoshi=contenttd.find('td',width='140').next_sibling.next_sibling.next_sibling.next_sibling.text#定位宣讲学校教室信息
        row.append(mouth)
